  ...
  function initGraph() {
    ...
    async function renderGraph(graph, fullSlug, renderGeneration) {
      ...
      function renderLabels() {
        ...
        var activeScale = defaultScale * 1.15
        var defaultAlpha = 1
        ...
        for (var i = 0; i < nodeRenderData.length; i++) {
          ...
          if (isCurrent || isHovered) {
            ...
            nodeData.label.alpha = nodeData.active ? 1 : 0.45
            nodeData.label.scale.set(defaultScale)
          } else {
            nodeData.label.alpha = defaultAlpha
            nodeData.label.scale.set(defaultScale)
          }
        }
      }
      ...
      for (var i = 0; i < nodes.length; i++) {
        ...
        var label = new PIXI.Text({
          ...
          style: {
            ...
            fill: isCurrentNode ? secondary : dark,
            fontFamily: bodyFont,
        ...
        if (isTagNode) {
          ...
          gfx.stroke({ width: 3, color: secondary })
        }
